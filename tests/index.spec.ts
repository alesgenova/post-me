...
function bindPostMessageToSource(target: Window, source: Window) {
  ...
  return function(message: any, targetOrigin: string) {
    ...
    setTimeout(() => {
      fireAnEvent("message", target, MessageEventJSDOM, { source, origin: source.origin, data: message });
    }, 0);
  }
}
...
function makeHandshake(_windows?: [Window, Window]) {
  let windows = _windows || makeWindows('https://example.com', 'https://child.example.com');
  const [parentWindow, childWindow] = windows;

  const handshakes = [
    ParentHandshake<ParentMethods, ParentEvents, ChildMethods, ChildEvents>(childWindow.origin, childWindow, parentMethods, parentWindow),
    ChildHandshake<ChildMethods, ChildEvents, ParentMethods, ParentEvents>(parentWindow.origin, childMethods, childWindow),
  ] as const;
  ...
  return new Promise<void>((resolve, reject) => {
    makeHandshake().then(([parentConnection, childConnection]) => {
      ...
      // Code in the parent app
      {
        ...
        const task0 = remoteHandle.call('foo', ...args0)
          .then(async value => {
        ...
        const task1 = remoteHandle.call('bar', ...args1)
          .then(async value => {
      ...
      // Code in the child app
      {
        ...
        const task0 = remoteHandle.call('baz', ...args0)
          .then(async value => {
        ...
        const args1 = [] as const;
        const task1 = remoteHandle.call('wut', ...args1)
          .then(async value => {
...
test('emit', () => {
  return new Promise<void>((resolve, reject) => {
    makeHandshake().then(([parentConnection, childConnection]) => {
      ...
      const messageFromParent: ParentEvents['opened'] = {type: 'foo', message: 'bar'};;
      const messageFromChild: ChildEvents['tapped'] = 'Emitting an important message!';
  ...
  return new Promise<void>((resolve, reject) => {
    makeHandshake().then(([parentConnection, childConnection]) => {
      ...
      // Code in the parent app
      {
        ...
        const task0 = remoteHandle.call('throws', ...args0)
          .then(_value => {
        ...
        const task1 = remoteHandle.call('whatever' as any, ...[])
          .then(_value => {
            throw new Error("It shouldn't have resolved");
          }).catch((e: Error) => {
            expect(e.message).toEqual(`The method "whatever" has not been implemented.`);
            return e;
          });
        tasks.push(task1);
      }
      ...
      // Code in the child app
      {
        ...
        // The method doesn't exist (typescript would actually catch the mistake during dev)
        const task1 = remoteHandle.call('anything' as any, ...[])
          .then(_value => {
            throw new Error("It shouldn't have resolved");
          }).catch((e: Error) => {
            expect(e.message).toEqual(`The method "anything" has not been implemented.`);
            return e;
          });
        tasks.push(task1);
      }
...
test('handshake-fail', () => {
  return new Promise<void>((resolve, reject) => {
    ...
    const parentHandshake = ParentHandshake(childWindow.origin, childWindow, {}, parentWindow);
...
test('multi-connection', () => {
  return new Promise<void>((resolve, reject) => {
    ...
    const [parentWindow, child0Window] = makeWindows(parentOrigin, child0Origin);

    const child1Window: Window = (new JSDOM(``, {url: child1Origin})).window as any;
    child1Window.postMessage = bindPostMessageToSource(child1Window, parentWindow);

    const maxRunTime = 500;
    const nEmits = 4;

    makeHandshake([parentWindow, child0Window])
      .then(([parent0Connection, child0Connection]) => {
        parentWindow.postMessage = bindPostMessageToSource(parentWindow, child1Window);
        makeHandshake([parentWindow, child1Window])
          .then(([parent1Connection, child1Connection]) => {
            const tasks: Promise<any>[] = [];
            { // Parent Code
              { // Child 0 connection
                ...
                const task0 = remoteHandle.call('ping', 'child0')
                  .then(res => {
                ...
                setTimeout(() => {
                  for (let i = 0; i < nEmits; ++i) {
                    localHandle.emit('opened', {type: 'foo', message: 'child0'});
                  }
                }, 50);
              }
              { // Child 1 connection
                ...
                setTimeout(() => {
                  for (let i = 0; i < nEmits; ++i) {
                    localHandle.emit('opened', {type: 'foo', message: 'child1'});
                  }
                }, 50);
              }
            }
